...
const UI = {
  ...
  estadoSync(estado, texto) {
    ...
    // lucide.createIcons() sustituye el <i data-lucide="..."> original por un
    // <svg>, así que ya no existe ningún <i> para reutilizar. En su lugar,
    // se crea un <i> nuevo con el ícono deseado y se reemplaza lo que haya
    // (sea <i> o <svg>) antes de volver a llamar a refrescarIconos().
    const iconoActual = el.querySelector("i, svg");
    const iconoNuevo = document.createElement("i");
    iconoNuevo.setAttribute("data-lucide", iconos[estado] || "check-circle-2");
    if (iconoActual) {
      iconoActual.replaceWith(iconoNuevo);
    } else {
      el.prepend(iconoNuevo);
    }
